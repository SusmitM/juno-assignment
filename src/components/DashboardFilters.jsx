...
const DashboardFilters = ({ searchInput, setSearchInput, selectedTab,triggerFilterValue, setTriggerFilterValue,actionFilterValue, setActionFilterValue }) => {
  ...
  return (
    <div className="mt-9 flex flex-col sm:flex-row gap-3">
      ...
      {selectedTab === "Completed" && (
        <div className="relative flex flex-col items-center rounded-lg mx-3 px-2 w-44">
            <button className="bg-gray-100 text-slate-500 p-2 w-full items-center justify-between font-medium text-sm rounded-lg flex gap-1.5" onClick={()=>setOpenActionReason(prev=>!prev)}>
                {actionFilterValue?actionFilterValue:"Action Reason"} {openActionReason?<IoIosArrowUp/>:<IoIosArrowDown/>}
            </button>

        {openActionReason&&
        <div className="bg-white absolute top-10 p-2 text-slate-500 font-medium text-sm w-full border border-slate-200 rounded-lg ">

          <div onClick={()=>{setActionFilterValue(null);setOpenActionReason(false)}} className="p-1.5 cursor-pointer hover:bg-indigo-100 hover:text-black rounded-lg">
                <h3>Reset</h3>
              </div>
          {
           ActionReasonValues.map((value,index)=>(
              <div onClick={()=>{setActionFilterValue(value);setOpenActionReason(false)}} className="p-1.5 cursor-pointer hover:bg-indigo-100 hover:text-black rounded-lg" key={index}>
                <h3>{value}</h3>
              </div>
            ))
          }
